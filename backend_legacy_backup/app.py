...
from .api.routes import debug_diag
...
async def lifespan(app: FastAPI):
    ...
    try:
        await init_db()
        # Log database URL (hide password if present)
        try:
            url = engine.url
            url_str = str(url)
            if getattr(url, 'password', None):
                url_str = url_str.replace(url.password, '***')
            logger.info(f"[DB] Connected to {url_str}")
        except Exception:
...
app.include_router(debug_diag.router, tags=["Debug"])
