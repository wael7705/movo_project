...
class OrderLifecycleService:
    ...
    async def create_order_stage(
        self, order_id: int, stage_name: str, start_time: Optional[datetime] = None
    ) -> OrderStageDuration:
    ...
    async def complete_order_stage(
        self, order_id: int, stage_name: str, end_time: Optional[datetime] = None
    ) -> OrderStageDuration:
    ...
    async def get_order_durations(self, order_id: int) -> Dict[str, Any]:
        ...
        query = (
            select(OrderStageDuration)
            .where(OrderStageDuration.order_id == order_id)
    ...
    async def calculate_processing_time(self, order_id: int) -> Dict[str, Any]:
        ...
        query = (
            select(Order)
            .options(selectinload(Order.stage_durations))
    ...
    async def validate_timestamps(self, order_id: int) -> Dict[str, bool]:
        ...
        if "preparing" in stages and "out_for_delivery" in stages:
            if (
                stages["preparing"]["end_time"]
                > stages["out_for_delivery"]["start_time"]
            ):
        ...
        if "out_for_delivery" in stages and "delivered" in stages:
            if (
                stages["out_for_delivery"]["end_time"]
                > stages["delivered"]["start_time"]
            ):
    ...
    async def update_order_status(
        self, order_id: int, new_status: OrderStatusEnum
    ) -> Order:
        ...
        if (
            ...
            and old_status == OrderStatusEnum.PENDING
        ):
            ...
            and old_status == OrderStatusEnum.ACCEPTED
        ):
            ...
            and old_status == OrderStatusEnum.PREPARING
        ):
            ...
            and old_status == OrderStatusEnum.OUT_FOR_DELIVERY
        ):
    ...
    async def get_orders_with_durations(
